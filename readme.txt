...
### Текущий прогресс
**Статус**: Дни 1-4, 6.1, 6.2, 6.3 и 7.1 завершены полностью.
**Новые вехи (День 6.3 и 7.1)**:
- **On-Demand Paging (День 6.3)**: Реализован обработчик Page Fault (INT 14). Lazy Allocation для виртуального диапазона `0xD0000000`. Zero-filled pages (безопасность от утечки данных). Автоматическое выделение физической памяти по требованию (CR2 + Error Code).
- **Многозадачность Ring 0 (День 7.1)**:
  - Process Control Block (PCB) и Circular Doubly Linked List (Ready Queue).
  - Ассемблерный Context Switch (`switch_context.asm`) с сохранением callee-saved регистров.
  - Магия Stack Forging (подделка начального стека для новых задач).
  - Task Wrapper (Трамплин) для безопасного старта задач с включенными прерываниями.
  - Интеграция Preemption (вытеснения) в PIT (IRQ0). Round-Robin планировщик.
