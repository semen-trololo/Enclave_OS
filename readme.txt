...
# ИТОГИ ДНЯ 14: Process Management & Supervisor Trees
...
8. Подготовка Copy-on-Write: добавлен OS-специфичный флаг PAGE_COW (9-й бит PTE), реализовано клонирование списка VMA (vma_clone) и создана базовая структура task_fork с математическим копированием Kernel Stack и обнулением EAX для ребенка.

## 🛑 Текущий блокпост (Задача на завтра):
При клонировании адресного пространства в функции vmm_clone_address_space возникает FATAL PAGE FAULT на чистом физическом адресе (например, 0x00102c00) вместо виртуального адреса Higher Half (0xC0102c00). Ядро падает при попытке прочитать PTE родителя в цикле клонирования.

Гипотезы для отладки:
1. Undefined Behavior (UB) при оптимизации GCC (-O2) в выражениях приведения физических адресов к виртуальным указателям (компилятор может "сломать" математику с 0xC0000000).
2. Проблемы с раскрытием макроса PHYS_TO_VIRT или линковка закэшированного старого объектного файла.
3. Отсутствие инвалидации TLB (или сброса CR3) для родителя перед чтением его Page Tables, из-за чего CPU использует устаревшие записи.
4. Потенциальный Race Condition, если аппаратное прерывание (PIT) происходит в середине цикла клонирования до завершения маппинга.

## 🎯 Следующие шаги (сразу после фикса бага):
1. Успешный запуск user-space теста test_fork.elf из Shell.
2. Проверка аппаратного срабатывания CoW Page Fault (ребенок пишет в shared_var, ядро перехватывает Fault и выделяет личную копию страницы).
3. Фиксация SSOT-синхронизации номеров syscall (SYS_GETPID = 20).
4. Переход к Дню 15 (Time & Sysinfo syscalls) или начало подготовки к портированию TinyCC.

Хорошего отдыха, коллега! Завтра мы вскроем этот FATAL PAGE FAULT, посмотрим на ассемблерный вывод GCC и добьем Copy-on-Write. 🍻
