...
Эти пробелы в архитектуре были выявлены в ходе анализа безопасности User Space и механизма Demand Paging. Без их устранения запуск ELF-бинарников, изоляция процессов и динамическое выделение памяти будут небезопасными или невозможными.
💣 Подводный камень №1: Отсутствие структуры VMA в task_t

    Проблема: В текущей PCB (task_t) отсутствует карта легальных диапазонов виртуальной памяти процесса. Обработчик Page Fault (INT 14) не может отличить "ленивое выделение памяти" (Demand Paging) от "попытки чтения чужой/несуществующей памяти" (Segfault/Exploit).
    Почему это критично: Без VMA ELF Loader на Дне 9 не сможет зарегистрировать сегменты .text и .data как легальные. Любое обращение к ним вызовет либо Kernel Panic (если обработчик строгий), либо бесконтрольную раздачу физической RAM хакеру (если обработчик наивный).
    Реализация (Обязательно к Дню 9):
        Создать include/vma.h и vma.c.
Добавить поле vma_node_t* vma_list; в task_t.
Модифицировать page_fault_handler(): перед выделением физической страницы сверять fault_addr (CR2) с current_task->vma_list. Если адрес вне VMA → SIGSEGV / возврат -EFAULT.

💣 Подводный камень №2: Отсутствие механизма sys_brk / sys_mmap

    Проблема: В Roadmap (День 9) заявлены sys_fork и sys_exec, но отсутствуют системные вызовы для динамического управления виртуальной памятью процесса. Пользовательский malloc() не сможет запросить расширение Heap.
    Почему это критично: Без sys_brk процесс не сможет легально "забронировать" виртуальный адрес. Demand Paging не сработает, так как VMA не будет содержать запрошенный диапазон. Программа упадет при первом же динамическом выделении памяти.
    Реализация (Обязательно к Дню 9):
        Добавить sys_brk(uint32_t new_brk) в таблицу системных вызовов (syscall.c).
        Логика sys_brk:
            Проверить, что new_brk ≥ текущего heap_start и не пересекается со Stack VMA.
            Найти Heap VMA ноду в vma_list и обновить её end = ALIGN_UP(new_brk, PAGE_SIZE).
            НЕ вызывать pmm_alloc_page()! Вернуть новый адрес пользователю.
            Физическая память появится автоматически при первом обращении пользователя к новой области (INT 14 → Demand Paging).
        Подготовить stub-обертку для будущей пользовательской malloc(), которая будет дергать sys_brk.
...
Этот раздел содержит концепции, требующие глубокой теоретической проработки и аккуратной реализации. Они не блокируют День 9, но критически важны для безопасности, производительности и соответствия промышленным стандартам (Дни 10–11).
💣 Подводный камень №3: Игнорирование Copy-on-Write (CoW) в VMA

    Проблема: В Roadmap (День 11) заявлена оптимизация fork() через CoW, но текущая архитектура VMA и Page Fault Handler не поддерживают флаги и логику отложенного копирования. PMM использует битмап без счетчиков ссылок.
    Почему это важно: Без CoW sys_fork() будет вынужден копировать всё адресное пространство родителя (десятки мегабайт), что убивает производительность и быстро истощает RAM. Без флагов в VMA обработчик INT 14 не отличит "попытку записи в Read-Only CoW страницу" от "нелегального Segfault".
    Задачи для проработки:
        Изучить алгоритм клонирования Page Directory и Page Tables при sys_fork().
        Расширить PMM: добавить массив uint8_t page_ref_count[] или хеш-таблицу для отслеживания общих физических страниц.
        Понять механизм снятия бита PAGE_WRITABLE в PTE для обоих процессов при сохранении VMA_FLAG_WRITE в VMA (логическая запись разрешена, физическая заблокирована для триггера CoW).
        Спроектировать логику COW Fault в page_fault_handler():
            Условие: if (!(pte & PAGE_WRITABLE) && (vma->flags & VMA_FLAG_COW))
            Действие: Аллоцировать новую страницу → k_memcpy (4KB) → Обновить PTE (новая страница + PAGE_WRITABLE) → Уменьшить ref_count старой страницы. Если ref_count == 0 → вернуть страницу в PMM.

💣 Подводный камень №4: Отсутствие Exception Tables (Extable) и copy_from_user

    Проблема: Текущая защита is_user_pointer() проверяет адрес ДО чтения (Time-of-Check). Однако статус страницы может измениться ДО использования (Time-of-Use), либо пользователь может передать адрес Guard Page / Swap-страницы. При чтении такого адреса из Ring 0 произойдет Page Fault, который наивный обработчик интерпретирует как баг ядра → Kernel Panic.
    Почему это важно: Без Extable любой кривой указатель из Ring 3 или защита стека (Guard Page) обрушит всю ОС. Ядро должно уметь "элегантно падать" внутри системного вызова, возвращая -EFAULT, а не паниковать. Это закрывает уязвимость TOCTOU.
    Задачи для проработки:
        Изучить синтаксис GCC inline-assembly для кастомных секций линкера (.section .extable, .previous).
        Спроектировать структуру записи: struct exception_table_entry { uint32_t insn; uint32_t fixup; };.
        Обновить linker.ld: собрать все записи .extable в непрерывный массив, экспортировать символы __start___extable и __stop___extable.
        Реализовать макрос copy_from_user() / copy_to_user(), который помечает опасные инструкции mov для линкера и генерирует fixup-код возврата ошибки.
        Модифицировать INT 14 (Kernel Mode Page Fault): бинарным поиском искать regs->eip в массиве .extable. Если найдено → подменить regs->eip на fixup (возврат -EFAULT) и выполнить iret.
        Изучить концепцию TOCTOU (Time-of-Check to Time-of-Use) и понять, почему аппаратная проверка внутри INT 14 надежнее программной проверки в C-коде до начала копирования.
