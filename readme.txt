...
### Текущий прогресс
**Статус**: Дни 1-8.1 завершены полностью. Ядро поддерживает вытесняющую многозадачность, аппаратную изоляцию памяти (CR3 Switch), Lazy FPU Switching, полноценную VFS и RAM-диск (Initrd).

**Новые вехи (День 8.1: Virtual File System и Initrd)**:
- **Архитектура VFS**: Реализован абстрактный полиморфный слой. Дерево каталогов построено на паттерне LCRS (Left-Child Right-Sibling) для O(1) добавления детей и полного отказа от `realloc`.
- **Трехзвенная модель File Descriptors (POSIX)**: Индустриальный стандарт. Глобальные `vfs_node_t` (Inodes), сессионные `open_file_t` (хранят `offset` и `ref_count` для поддержки `fork`/`dup`) и локальные `fd_table` внутри PCB (`task_t`).
- **Initrd (TAR UStar Parser)**: Написан парсер, который берет GRUB-модуль из физической памяти и разворачивает его в `tmpfs` прямо в RAM. Реализовано корректное отсечение префикса `./` и автоматическое создание промежуточных директорий.
- **Ring-Based Access Control (RBAC)**: Внедрен флаг `FS_SYSTEM`. Ядро использует аппаратную проверку CPL (Current Privilege Level через регистр CS). Ring 0 (Kernel/Shell) имеет доступ ко всему, Ring 3 (User Space) жестко блокируется при попытке чтения системных файлов (возврат `VFS_EACCES`).
- **POSIX Syscalls & Shell**: Реализованы `sys_open`, `sys_close`, `sys_read`, `sys_write`, `sys_readdir`. Команды `ls` и `cat` интегрированы в Shell и работают через полноценную границу системных вызовов.
- **Исправление критических багов Дня 7/8**:
  - Устранена фатальная передача физического адреса в TSS ESP0 (приводила бы к Page Fault из Ring 3).
  - Решена проблема Stack Forging (16-byte ABI alignment).
  - Инициализация FD 0,1,2 (stdin/stdout) для `main_task`, предотвратившая перезапись стандартных потоков.
  - Настроено наследование флагов безопасности (`FS_SYSTEM`) для вложенных директорий.

---
[ДЕНЬ 8] Файловая система, VFS и Storage (УТВЕРЖДЕННАЯ АРХИТЕКТУРА)
[ДЕНЬ 8.1: ЗАВЕРШЕНО] VFS Layer, Initrd (TAR) и POSIX Syscalls
- Архитектура VFS (Virtual File System): Абстрактный слой, скрывающий
разницу между RAM-диском, FAT32 и устройствами. Полиморфизм на C
реализован через структуру vfs_node_t (Inode) с указателями на функции
(read, write, readdir, finddir) и полем void* private_data для контекста бэкенда.
- Трехзвенная модель File Descriptors (Индустриальный стандарт):
1. vfs_node_t: Глобальное представление файла. Уникален, НЕ хранит offset.
2. open_file_t: Создается при каждом sys_open(). Хранит курсор (offset),
флаги (O_RDONLY) и ref_count. Позволяет открывать один файл многократно.
3. fd_table: Массив указателей на open_file_t внутри PCB (task_t).
Целое число (FD) — это просто индекс в этом массиве.
- Дерево каталогов: Используется паттерн Left-Child Right-Sibling
(first_child, next_sibling) для избежания динамических массивов и realloc.
- Безопасность и Права (Ring-Based Access Control): Отказ от сложных UID/GID.
Введен флаг FS_SYSTEM. Внутренние вызовы ядра (vfs_findnode) игнорируют
его, позволяя ядру читать системные бинарники (ELF). Системные вызовы
из Ring 3 (sys_open) аппаратно проверяют CPL (через регистр CS) и возвращают EACCES, жестко
изолируя User Space от внутренностей ядра.
- Initrd (TAR UStar): Загружается через GRUB Modules. Парсится в памяти
(tmpfs). Системные директории (/boot) помечаются FS_SYSTEM и Read-Only.
Пользовательские директории (/tmp) работают как Writable Sandbox в Heap.
- POSIX Syscalls: sys_open, sys_read, sys_write, sys_close, sys_readdir.
Магия маршрутизации: sys_write для FD=1 (stdout) через VFS автоматически
вызывает драйвер serial.

[ДЕНЬ 8.2] ATA PIO Driver и FAT32 Read-Only (ОТЛОЖЕНО НА ПОСЛЕ ДНЯ 9)
- Архитектурное решение: ATA PIO и поддержка реальных дисков отложены до стабилизации User Space. Текущих возможностей Initrd достаточно для загрузки ELF-бинарников.
========================================================================
...
========================================================================
### АРХИТЕКТУРНЫЕ ВАЛИДАЦИИ И ИСПРАВЛЕНИЯ (ДЕНЬ 7)

35. RETURN PATH UNWINDING (ВЫТЕСНЯЮЩАЯ МНОГОЗАДАЧНОСТЬ)
При вызове schedule() из IRQ0 (PIT) флаг need_resched не требуется.
МЕХАНИЗМ: switch_context подменяет ESP. При возврате к задаче стек
естественно "раскручивается" (ret -> schedule -> pit_handler -> iret),
корректно восстанавливая аппаратный контекст прерывания без хака стека.

36. TSS ESP0 И ВИРТУАЛЬНЫЕ АДРЕСА (КРИТИЧНО)
В schedule() при вызове tss_set_kernel_stack() ОБЯЗАТЕЛЬНО передавать
виртуальный адрес: PHYS_TO_VIRT(new_task->kernel_stack + 4096).
ПРОБЛЕМА: Передача физического адреса приведет к Page Fault при первом
же прерывании из Ring 3, так как MMU ищет стек через текущий CR3.

37. УТЕЧКА ПАМЯТИ ЗОМБИ-ПРОЦЕССОВ (REAPER MECHANISM)
task_exit() НЕ МОЖЕТ вызывать pmm_free_page() для Page Directory и стека,
так как регистры CR3 и ESP процессора всё ещё указывают на них.
РЕШЕНИЕ: Освобождение памяти (vmm_destroy_address_space) должно происходить
строго ПОСЛЕ switch_context() внутри schedule() для задачи со статусом DEAD.

38. K_SLEEP И ПЕРЕПОЛНЕНИЕ UINT32 (TIME_AFTER)
Сумма start + ms вызывает overflow через ~49 дней аптайма.
РЕШЕНИЕ: Использовать математику разности: while ((uint32_t)(tick_count - start) < ms).

39. ВЫРАВНИВАНИЕ СТЕКА ДЛЯ USER SPACE (SSE/ABI)
Stack Forging в task_create должен учитывать 16-байтное выравнивание стека
перед вызовом функции (System V ABI). Требуются dummy-пуши на пустой стек,
чтобы перед первой инструкцией user-кода ESP был кратен 16.
========================================================================
...
========================================================================
### ПЛАН РЕАЛИЗАЦИИ: ДЕНЬ 8 И ДЕНЬ 9 (УТВЕРЖДЕННАЯ АРХИТЕКТУРА)

[========================================================================
[ДЕНЬ 8] Файловая система, VFS и Storage (УТВЕРЖДЕННАЯ АРХИТЕКТУРА)

[ДЕНЬ 8.1] VFS Layer, Initrd (TAR) и POSIX Syscalls
- Архитектура VFS (Virtual File System): Абстрактный слой, скрывающий
  разницу между RAM-диском, FAT32 и устройствами. Полиморфизм на C
  реализован через структуру vfs_node_t (Inode) с указателями на функции
  (read, write, readdir, finddir) и полем void* private_data для контекста бэкенда.
...
- Безопасность и Права (Ring-Based Access Control): Отказ от сложных UID/GID.
  Введен флаг FS_SYSTEM. Внутренние вызовы ядра (vfs_findnode) игнорируют
  его, позволяя ядру читать системные бинарники (ELF). Системные вызовы
  из Ring 3 (sys_open) проверяют этот флаг и возвращают EACCES, жестко
  изолируя User Space от внутренностей ядра.
- Initrd (TAR UStar): Загружается через GRUB Modules. Парсится в памяти
  (tmpfs). Системные директории (/boot) помечаются FS_SYSTEM и Read-Only.
  Пользовательские директории (/tmp) работают как Writable Sandbox в Heap.
- POSIX Syscalls: sys_open, sys_read, sys_write, sys_close, sys_readdir.
  Магия маршрутизации: sys_write для FD=1 (stdout) через VFS автоматически
  вызывает драйвер framebuffer или serial.

[ДЕНЬ 8.2] ATA PIO Driver и FAT32 Read-Only
- ATA PIO Driver: Работа с портами 0x1F0-0x1F7. LBA28 (до 128 ГБ).
  Функции ожидания BSY/DRQ и чтения сырых секторов (512 байт).
- MBR и Partition Table: Чтение LBA 0, поиск первого активного раздела.
- FAT Auto-Detection: Парсинг BIOS Parameter Block (BPB). Динамическое
  определение FAT12/16/32 по размеру кластеров и BPB_FATSz32. Защита от
  ловушки mkfs.fat (которая создает FAT16 для образов <33 МБ).
- FAT32 RO & LFN (VFAT): Обход цепочек кластеров. Парсинг Long File Names
  (UCS-2/UTF-16LE) с конвертацией в UTF-8 для VFS.
- VFS Mount: Флаг FS_MOUNTPOINT. При обходе пути vfs_findnode делает
  "телепортацию" на корневой vfs_node_t примонтированной ФС.

АРХИТЕКТУРНЫЕ КОНТРАКТЫ ДНЯ 8 (ЗАПРЕЩЕНО НАРУШАТЬ):
- VFS НИКОГДА не знает о FAT32, ATA или RAM. Только через указатели на функции.
- Драйверы блочных устройств (block_device_t) работают ТОЛЬКО с LBA.
- Файловые системы работают ТОЛЬКО с block_device_t.
- File Descriptors принадлежат процессу (в PCB), а не ядру глобально.
- Проверка прав доступа (FS_SYSTEM) происходит ТОЛЬКО на границе Syscall
  (Ring 3 -> Ring 0). Внутренний код ядра проверок не делает.
========================================================================

[ДЕНЬ 9] User Space и ELF Loader (Послезавтра)
9.1 ELF Format Parser: Парсинг ELF Header (e_entry, e_phoff, e_phnum).
    Обработка Program Headers типа PT_LOAD (p_vaddr, p_filesz, p_memsz, p_offset).
9.2 ELF Loader: Выделение User Space памяти через vmm. Чтение сегментов
    .text и .data из vfs_node_t (Initrd) в виртуальное адресное пространство
    процесса. Настройка точки входа (entry point).
9.3 /init как PID 1: По Linux-традиции, первой запускаемой программой
    становится /init из Initrd. Она получает argc/argv и становится родителем
    для всех последующих процессов. Реализация fork/exec в будущем.

АРХИТЕКТУРНЫЕ КОНТРАКТЫ (ЗАПРЕЩЕНО НАРУШАТЬ):
- VFS НИКОГДА не знает о FAT32, ATA или RAM. Только через указатели на функции.
- Драйверы блочных устройств (block_device_t) работают ТОЛЬКО с LBA (сырыми
  секторами 512 байт). Они НЕ знают про файловую систему.
- Файловые системы (FAT32, ext2) работают ТОЛЬКО с block_device_t.
- File Descriptors принадлежат процессу (в PCB), не ядру глобально.
- Initrd распаковывается В ПАМЯТЬ (Heap) при старте. Это tmpfs, а не диск.
========================================================================
