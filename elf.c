...
uint32_t elf_load(vfs_node_t* file_node, task_t* task) {
    ...
    for (uint16_t i = 0; i < elf_header.e_phnum; i++) {
        ...
        // ✅ ИСПРАВЛЕНО: Создание VMA для сегмента
        ...
        vma_add(task, phdr.p_vaddr, phdr.p_vaddr + phdr.p_memsz, vma_flags);
        ...
        for (uint32_t addr = start_page; addr < end_page; addr += 4096) {
            ...
            vmm_map_page_in_pd(task->pdir_virt, addr, phys, pte_flags);
        }
